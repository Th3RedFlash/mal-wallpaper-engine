# main.py (v1.11 - Fixed SyntaxError in process_anime_with_semaphore)
...
HEADERS = { "User-Agent": "MAL_Wallpaper_Engine/1.11 (Fix Syntax; Contact: YourEmailOrProjectURL)" } # Updated version
...
async def get_wallpapers(username: str, search: str = None, max_per_anime: int = 5): # Default 5 images
    ...
    with LIMITED CONCURRENCY and Backend Duplicate Filtering. Returns up to max_per_anime.
    """
    ...
    # ... (MAL Fetching Logic - Remains the same as v1.10 with detailed logging) ...
    ...
    try:
        ...
        if content_type_check:
            ...
            try:
                mal_data = response.json(); print(f"  MAL JSON Attempt - Successfully parsed JSON. Found {len(mal_data)} items in list.")
                processed_ids = set(); count = 0; print(f"  MAL JSON Attempt - Starting loop through items...")
                    ...
                    if isinstance(item, dict) and item.get('status') == 2:
                        ...
                        if title and anime_id not in processed_ids:
                            if not search or search.lower() in title.lower():
                                eng_title = item.get('anime_title_eng'); title_eng = eng_title if eng_title and eng_title.lower() != title.lower() else None
            ...
            except json.JSONDecodeError as e_json: last_error_message = f"MAL JSON Attempt - Error decoding JSON: {e_json}"; print(f"  {last_error_message}"); print(f"      Response text that failed parsing: {response.text[:500]}")
            except Exception as e_loop: last_error_message = f"MAL JSON Attempt - Error during item processing loop: {e_loop}"; print(f"  {last_error_message}"); traceback.print_exc()
        else: last_error_message = f"MAL JSON endpoint did not return 'application/json'. Actual: {actual_content_type}"; print(f"  MAL JSON Attempt - {last_error_message}")
    ...
    if not mal_data_fetched_successfully:
        # ... (Full HTML fallback logic with detailed logging - unchanged from v1.10) ...
    ...
    # *** CORRECTED HELPER FUNCTION SYNTAX ***
    async def process_anime_with_semaphore(sem, anime_data, index):
        ...
        # Use 'async with' on its own line
        async with sem:
            ...
            # Increment counter after processing is done for this item
            processed_titles_count += 1
            return result_tuple # Return tuple (original_title, final_url_list, simplified_term_used)
    # *** END CORRECTION ***
    ...
    final_results = {}; processed_simplified_terms = set(); skipped_count = 0; title_to_info_map = {item['title']: item for item in unique_anime_list}
    ...
    print(f"\nFinished all processing. Found wallpapers for {len(final_results)} titles.") # Uses correct variable
